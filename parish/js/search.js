...
(function () {
  ...
  function parishSlug(p) {
    ...
    var name = (p.name || "parish").toLowerCase();
    return name.replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80);
  }
  ...
  Promise.all([
    ...
    // Last 10 fetches are zip_coords shards; rest are parish arrays
    ...
    // Unlock search immediately; fill any remaining coords in background
    finishReady();
